...
void InsertionSortHelper(int *data, int low, int high);
...
int main (int argc, char *argv[]) {
   ...
   // Establish the arrays local to each process
    ...
    if (rank == 0) {
        ...
        InsertionSortHelper(local_data, 0, num_local_elements - 1);
    } else {
        // Local insertion sort
        InsertionSortHelper(local_data, 0, num_local_elements - 1);
    }
...
void Print(int *data, int size) {
    ...
    std::cout << "Begin Data for MPI Insertion Sort" << std::endl;
    // Loop through and print the data
    for (int i = 0; i < size; i++) {
        std::cout << data[i] << std::endl;
    }
    std::cout << "End Data for MPI Insertion Sort" << std::endl;
}

// Report timing
void ReportTiming(int size, int numCores, double exec_time_ms) {
    std::cout << "The most recent run used parallel MPI Insertion sort to sort " <<
