...
void MergeSortHelper(int *data, int start, int end);
...
int main (int argc, char *argv[]) {
   ...
   // Establish the arrays local to each process
    ...
    if (rank == 0) {
        ...
        // Local merge sort
        MergeSortHelper(local_data, 0, num_local_elements - 1);
    } else {
        // Local merge sort
        MergeSortHelper(local_data, 0, num_local_elements - 1);
    }
...
void Print(int *data, int size) {
    ...
    std::cout << "Begin Data for MPI Merge Sort" << std::endl;
    // Loop through and print the data
    for (int i = 0; i < size; i++) {
        std::cout << data[i] << std::endl;
    }
    std::cout << "End Data for MPI Merge Sort" << std::endl;
}
...
void MergeSortHelper(int *data, int start, int end) {
    if (start < end) {
        ...
        MergeSortHelper(data, start, middle);
        MergeSortHelper(data, middle + 1, end);

        // Merge the halves
        MergeHalves(data, start, middle, end);
    }
}
