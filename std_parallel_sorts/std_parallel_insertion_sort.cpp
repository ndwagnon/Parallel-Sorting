// Local includes
#include "std_parallel_insertion_sort.h"
...
// Standard Constuctor
StdParallelInsertionSort::StdParallelInsertionSort() {
...
StdParallelInsertionSort::StdParallelInsertionSort(int size, int min, int max, int numCores_a) {
    // Initialize all of the private data
    isFilled = false;
    isSorted = false;
    numCores = numCores_a;
...
void StdParallelInsertionSort::Sort() {
    // Start the timer
    auto t1 = std::chrono::high_resolution_clock::now();

    // Spawn the threads
    for (int i = 0; i < numCores; i++) {
        threads.push_back(std::thread(&StdParallelInsertionSort::InsertionSortThreadHelper, this, i));
    }
...
bool StdParallelInsertionSort::VerifySort() {
...
// Prints the data
void StdParallelInsertionSort::Print() {
    // Make sure data exists
    if (!isFilled) {
    ...
    std::cout << "Begin Data for Parallel Insertion Sort" << std::endl;
    // Loop through and print the data
    for (int i = 0; i < size_; i++) {
        std::cout << data[i] << std::endl;
    }
    std::cout << "End Data for Parallel Insertion Sort" << std::endl;
}
...
// Clear the data
void StdParallelInsertionSort::ClearData() {
    delete data;
    data = nullptr;
    size_ = 0;
    isFilled = false;
...
void StdParallelInsertionSort::SetNumCores(int numCores_a) { numCores = numCores_a; }

// Getters
bool StdParallelInsertionSort::IsSorted() { return isSorted; }
...
void StdParallelInsertionSort::InsertionSortThreadHelper(int tid) {
    ...
    InsertionSortHelper(low, high);
}

void StdParallelInsertionSort::FinalMerge(int thread_factor, int aggregation_factor) {
    if (numCores == 1)
        return;
    ...
    for (int i = 0; i < thread_factor; i += 2) {
        ...
        if (thread_factor > 2) {
            threads.at(i) = std::thread(&StdParallelInsertionSort::MergeHalves, this, left, middle, right);
        } else {
...
void StdParallelInsertionSort::InsertionSortHelper(int low, int high) {
    // Outer loop
    for (int i = low+1; i < high + 1; i++) {
        int temp = data[i];
        int j = i - 1;
        // Inner loop
        while (temp < data[j] && j >= low) {
            data[j+1] = data[j];
            --j;
        }
        data[j+1] = temp;
